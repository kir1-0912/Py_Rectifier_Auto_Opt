...
from TurBO import turbo_solver
import pickle
import torch
import os
...
def parallel_evaluate(X_np: np.ndarray) -> torch.Tensor:
    """
    Args:
        X_np: shape (n_samples, n_params)

    Returns:
        torch.Tensor: shape (n_samples, 1)
    """
    results = []
    for x in X_np:
        x_list = x.tolist()
        ...
        results.append(y)
    return torch.tensor(results, dtype=torch.float64).view(-1, 1)




turbo_solver(parallel_evaluate, bounds, N_INIT=8, BATCH_SIZE=8, NUM_RESTARTS=10, RAW_SAMPLES=512,  # N_INIT之前是64
                     N_CANDIDATES=4096, acqf="ts")



with open("turbo_optimization_history01.pkl", "rb") as f:
...
plt.title("TuRBO Optimization Progress")
plt.grid(True)
plt.show()
# bo_solver(
#     funct=Object,
