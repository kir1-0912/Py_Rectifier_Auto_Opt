...
class TurboHParams:
    ...
    def from_dict(cls, tr_hparams: Dict) -> None:
        ...
        if unexpected_keys:
            print(
                warnings.warn(f"Got unexpected tr_hparams keys, ignoring: {unexpected_keys}")
            )
...
class TrustRegion(ABC, Module):
            ...
            if update_streaks:
                ...
                if self.n_successes >= self.tr_hparams.success_streak:
                    ...
                    if self.length < self.tr_hparams.length_min:
                        return True
        print("success count", self.n_successes)
        print("failure count", self.n_failures)
    ...
    def _has_improved_objective(
        ...
        if self.weights is None:
            raise ValueError("self.weights none")

        Y_all = self.Y_estimate
        Y_new = Y_all[-n_new:]

        scalarized_all = (Y_all * self.weights).sum(dim=-1)
        scalarized_new = (Y_new * self.weights).sum(dim=-1)

        if scalarized_all.shape[0] > n_new:
            best_so_far = scalarized_all[:-n_new].max()
        else:
            best_so_far = float("-inf")

        return scalarized_new.max() > best_so_far
...
class HypervolumeTrustRegion(TrustRegion):
    ...
    def _update_center_and_best_points(
        ...
        if X_center is not None:
            ...
            if hv_contributions is not None and hv_contributions.shape[0] > 0:
                indices_mask = torch.zeros_like(not_taken_mask)
                indices_mask[indices] = True
                eligible_mask = indices_mask & not_taken_mask
                if eligible_mask.any():
                    eligible_indices = eligible_mask.nonzero().view(-1).to(hv_contributions.device)
                    ...
                    # available point
                    not_taken_indices = not_taken_mask.nonzero().view(-1).to(hv_contributions.device)
    ...
    def compute_grouped_hv(self, Y, group_sizes, subset_size=5):
        """Compute hypervolume by grouping objectives and sampling subsets."""
        n_obj = Y.shape[1]
        idx = 0
        total_hv = 0.0
        ...
        for group_size in group_sizes:
            ...
            for _ in range(1):  # You can adjust the number of samples
                selected_idx = torch.randperm(group_size)[:subset_size]
                sub_Y = group_Y[:, selected_idx]
                ref_point = torch.full((subset_size,), -5.0, device=Y.device, dtype=Y.dtype)
    ...
    def _has_improved_objective(self, n_new: int) -> bool:
        ...
        if obj.shape[0] > 0:
            ...
            if pareto_mask[-(obj.shape[0]):].any():
                ...
                # check hypervolume improvement using grouped approach
                pareto_aug_obj = aug_obj[pareto_mask]
                better_than_ref = (pareto_aug_obj > self.ref_point).all(dim=-1)
                pareto_aug_obj = pareto_aug_obj[better_than_ref]

                if pareto_aug_obj.shape[0] > 0:
                    # Define how to group the objectives (example: [3, 2] for 5 objectives)
                    group_sizes = [pareto_aug_obj.shape[1]]  # Single group by default
                    # You can modify group_sizes based on your problem structure

                    new_hv = self.compute_grouped_hv(pareto_aug_obj, group_sizes)
                    print(f"New grouped HV: {new_hv}")
