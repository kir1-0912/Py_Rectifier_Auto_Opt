...
def exe_spectre_woxfmr(xnorm: list, RL=3e3, Fre=2.4, Pin=-5):
    ...
    C1 = xnorm[3]
    K = xnorm[4]
    params_str = f"{C2}_{N}_{L1}_{C1}_{K}_{Fre}_{Pin}_{RL}"
    hash_object = hashlib.md5(params_str.encode())
    hash_str = hash_object.hexdigest()
    short_hash_scs_filename = hash_str[:12]
    spectre_netlist = f"""
// Generated for: spectre
// Generated on: Aug  9 14:25:52 2025
// Design library name: rect
// Design cell name: rect_zy_FX_testbench
// Design view name: schematic
simulator lang=spectre
global 0
parameters K={K} C2={C2}f N={N} L1={L1}n RF={Fre}G RLOAD={RL} C1={C1}p Pin={Pin}
...
// Library name: rect
// Cell name: rect_zy_FX_testbench
// View name: schematic
M1 (net5 net21 VSS VSS) nch_lvt_mac l=30n w=1e-06*N multi=1 nf=N sd=100n \
...
M0 (net21 net5 VSS VSS) nch_lvt_mac l=30n w=1e-06*N multi=1 nf=N sd=100n \
        ad=((N-int(N/2)*2)*(7.5e-08+((N-1)*1e-07)/2+0)+(N+1-int((N+1)/2)*2)*((N/2)*1e-07))*1e-06 \
        as=((N-int(N/2)*2)*(7.5e-08+((N-1)*1e-07)/2+0)+(N+1-int((N+1)/2)*2)*(7.5e-08+7.5e-08+(N/2-1)*1e-07+0+0))*1e-06 \
        pd=(N-int(N/2)*2)*((7.5e-08+((N-1)*1e-07)/2+0)*2+(N+1)*1e-06)+(N+1-int((N+1)/2)*2)*(((N/2)*1e-07)*2+N*1e-06) \
        ps=(N-int(N/2)*2)*((7.5e-08+((N-1)*1e-07)/2+0)*2+(N+1)*1e-06)+(N+1-int((N+1)/2)*2)*((7.5e-08+7.5e-08+(N/2-1)*1e-07+0+0)*2+(N+2)*1e-06) \
        ...
        dfm_flag=0 spmt=1.11111e+15 spomt=0 spomt1=1.11111e+60 \
        spmb=1.11111e+15 spomb=0 spomb1=1.11111e+60
M3 (net21 net5 VLOAD VLOAD) pch_lvt_mac l=30n w=1e-06*1.2*N multi=1 \
        nf=1.2*N sd=100n \
...
M2 (net5 net21 VLOAD VLOAD) pch_lvt_mac l=30n w=1e-06*1.2*N multi=1 \
        nf=1.2*N sd=100n \
        ad=((1.2*N-int(1.2*N/2)*2)*(7.5e-08+((1.2*N-1)*1e-07)/2+0)+(1.2*N+1-int((1.2*N+1)/2)*2)*((1.2*N/2)*1e-07))*1e-06 \
        as=((1.2*N-int(1.2*N/2)*2)*(7.5e-08+((1.2*N-1)*1e-07)/2+0)+(1.2*N+1-int((1.2*N+1)/2)*2)*(7.5e-08+7.5e-08+(1.2*N/2-1)*1e-07+0+0))*1e-06 \
        pd=(1.2*N-int(1.2*N/2)*2)*((7.5e-08+((1.2*N-1)*1e-07)/2+0)*2+(1.2*N+1)*1e-06)+(1.2*N+1-int((1.2*N+1)/2)*2)*(((1.2*N/2)*1e-07)*2+1.2*N*1e-06) \
...
C24 (net38 net35) capacitor c=C2
C23 (net39 net21) capacitor c=C1
C22 (net37 net5) capacitor c=C1
C21 (net41 net40) capacitor c=C1
C20 (net34 net36) capacitor c=C1
C2 (VLOAD VSS) capacitor c=10p
PORT1 (VLOAD VSS) port r=RLOAD type=sine
PORT0 (net27 VSS) port r=50 num=1 type=sine freq=RF dbm=Pin
I1 (net27 VSS net34 net41) ideal_balun
L18 (net40 net35) inductor l=L1 q=10 fq=2.4G mode=1
L17 (net35 net39) inductor l=L1 q=10 fq=2.4G mode=1
L16 (net38 net37) inductor l=L1 q=10 fq=2.4G mode=1
L15 (net36 net38) inductor l=L1 q=10 fq=2.4G mode=1
K1 mutual_inductor coupling=K ind1=L18 ind2=L17
K0 mutual_inductor coupling=K ind1=L15 ind2=L16
...
def Rect_Opt_Flow(xnorm:list):
    try:
        ...
        with open("/home/ieda/wangl/PY/Py_Rectifier_Auto_Opt_WoXFMR/RESULTZY/result813.csv", mode="a+") as f:
            # f.write(
            ...
            f.write(
                f"{xnorm[0]}, {int(xnorm[1])},{xnorm[2]}, {xnorm[3]}, "
                f"{xnorm[4]},{fom_dict.get('fom1', -12)}, {rl_dict.get('rl1')},"
                f"{fom_dict.get('fom2', -12)}, {rl_dict.get('rl2')}\n")
    except Exception as e:
        print(f"[simwrong] give{e}")
        return None
    finally:
    ...
    processed_fom_list = [FoM if FoM is not None else -12 for FoM in fom_list]
