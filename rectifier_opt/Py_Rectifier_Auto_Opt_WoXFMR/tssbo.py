...
class tSubspace:
    ...
    def update_subspace(self, new_x, new_y, new_mean_f=None, GP_model_list=None, mean_and_std=None):
        ...
        lb1 = [10, 10, 1, 3, 1, 1, 3, 1, 1, 1, 1, 1, 0.1]
        ub1 = [300, 300, 50, 20, 20, 50, 20, 20, 10000, 10000, 6, 6, 0.85]
        ...
        self.sigma = torch.clamp(self.sigma, min=0, max=0.7)
...
class NARGPKernel(gpytorch.kernels.Kernel):
    ...
    # 作用于前
    # n - 1
    # 个维度，可以捕捉输入空间中大部分维度的一般模式。
    # rbf2 * rbf3
    # 的乘积核特别关注最后一个维度与其他维度的交互。
    # 增强模型灵活性：
    # 加性核允许模型同时捕捉全局模式和局部特征。
    # 这种结构可以处理输入特征中可能存在的不同尺度的变化。
    # 处理非平稳性：
    # 在某些问题中，最后一个维度可能具有特殊意义（例如，时间序列中的时间维度）。
    # 这种结构允许模型对最后一个维度给予特殊关注，同时不忽视其他维度。
    # 改善可解释性：
    # 可以分别分析两个部分的贡献，有助于理解不同特征对预测的影响。
    # 平衡复杂性和泛化能力：
    # 通过组合简单的核函数，可以创建复杂的非线性映射，同时避免过拟合。
    # 适应性强的结构：
    # 这种结构可以适应各种不同的问题，无论是高维空间中的平滑变化还是某些维度上的急剧变化。
    # 在实践中，这种核函数的设计特别适用于以下场景：
    #
    # 输入特征中存在明显的层次结构。
    # 某些特定维度（如最后一个维度）可能对输出有特殊影响。
    # 需要模型能够捕捉复杂的非线性关系，同时保持良好的泛化能力。
...
class CombinedKernel(Kernel):
    def __init__(self, alpha=0.5, gamma=1.0, indices_phys=[12, 9, 10, 11], indices_rbf=list(range(9)), **kwargs):
        super().__init__(has_lengthscale=False)
        self.alpha = alpha
        self.gamma = gamma
        self.indices_phys = indices_phys
        self.indices_rbf = indices_rbf

    def feature_mapping(self, x):
        V_diff = 2 * np.sqrt(10 ** (-5 / 10) / 1000 * 50 * 2)
        omega = 2 * math.pi * 2.4e9
        k, C2, L1, L2 = x[self.indices_phys]
        n = 1 / k * torch.sqrt(L1 / L2)
        L2_prime = L2 * (1 - k ** 2)
        C2_prime = (L1 / (k ** 2 * L2)) * C2
        numerator = (1 / (1j * omega * C2_prime)) * k * torch.sqrt(L2 / L1) * V_diff
        denominator = (1 / (1j * omega * C2_prime)) + 1j * omega * L2_prime
        V_rect = numerator / denominator
        return torch.stack([n.real, L2_prime.real, C2_prime.real, (V_rect.abs() ** 2).real])
    ...
    def forward(self, x1, x2, **params):
        ...
        for i in range(x1.size(0)):
            ...
            for j in range(x2.size(0)):
                phi1 = self.feature_mapping(x1[i])
                phi2 = self.feature_mapping(x2[j])
                k_phys = torch.dot(phi1, phi2)

                diff = x1[i][self.indices_rbf] - x2[j][self.indices_rbf]
                k_rbf = torch.exp(-self.gamma * torch.norm(diff) ** 2)

                k_comb = self.alpha * k_phys + (1 - self.alpha) * k_rbf
                row.append(k_comb)
...
def select_candidate_EI_unconstrained(
    ...
    kernel = CombinedKernel(alpha=0.5, gamma=1.0)
    train_y = (Y_torch - Y_torch.mean()) / (1e-6 + Y_torch.std())
    model = GP({'train_x': X_torch, 'train_y': train_y, 'train_sigma': None}, kernel=kernel)
    model.fit()
...
def bo_solver(funct, dim, bounds, init_x=None, init_y=None,
                 sigma=0.7, mu=0.5, c1=None, c2=None, allround_flag=False, greedy_flag=False,
